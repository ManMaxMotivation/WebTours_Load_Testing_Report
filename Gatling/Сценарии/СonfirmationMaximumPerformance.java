...
public class СonfirmationMaximumPerformance extends Simulation {
...
//62 минуты держим 120 пользователей.

  {
      setUp(
              UC1Login.injectClosed(
                      rampConcurrentUsers(0).to(12).during(Duration.ofSeconds(60)), // разгон 60 сек
                      constantConcurrentUsers(12).during(Duration.ofMinutes(62))     // 62 минуты нагрузка (2 мин стабилизации + 60 мин анализа)
              ),
              UC2Search.injectClosed(
                      rampConcurrentUsers(0).to(24).during(Duration.ofSeconds(60)),
                      constantConcurrentUsers(24).during(Duration.ofMinutes(62))
              ),
              ...
              UC4ViewTickets.injectClosed(
                      ...
                      constantConcurrentUsers(36).during(Duration.ofMinutes(62))
              ),
              UC5DeleteTickets.injectClosed(
                      rampConcurrentUsers(0).to(12).during(Duration.ofSeconds(60)),
                      constantConcurrentUsers(12).during(Duration.ofMinutes(62))
              ),
